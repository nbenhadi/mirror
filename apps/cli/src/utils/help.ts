...
function localize(text: string): string {
  return text
    .replace('[options]', `[${t('cmd.help.options')}]`)
    .replace('[command]', `[${t('cmd.help.commands')}]`)
...
function formatHelp(cmd: Command, helper: Help): string {
  ...
  lines.push(`${capitalize(t('cmd.help.usage'))}: ${usage}`)
  lines.push('')

  const argList = helper.visibleArguments(cmd)
  if (argList.length > 0) {
    lines.push(`${capitalize(t('cmd.help.arguments'))}:`)
  ...
  if (optionList.length > 0) {
    lines.push(`${capitalize(t('cmd.help.options'))}:`)
    for (const opt of optionList) {
      const desc = helper
        .optionDescription(opt)
        .replace(/\(default: ([^)]+)\)/, `(${t('cmd.help.default' as TranslationKey)}: $1)`)
      lines.push(`  ${helper.optionTerm(opt).padEnd(termWidth + 2)} ${desc}`)
    }
    lines.push('')
  }
  ...
  if (cmdList.length > 0) {
    lines.push(`${capitalize(t('cmd.help.commands'))}:`)

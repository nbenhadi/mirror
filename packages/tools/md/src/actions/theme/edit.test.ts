...
describe('themeEdit action', () => {
  it('returns the css path of an existing user theme', async () => {
    ...
    const result = await themeEdit({ action: 'theme.edit', name: 'sidebar' }, ctx)
  ...
  it('fails for a theme that does not exist', async () => {
    const result = await themeEdit({ action: 'theme.edit', name: 'does-not-exist' }, ctx)
    expect(result.success).toBe(false)
    if (!result.success) expect(result.error.code).toBe('NOT_FOUND')
  })

  it('fails for a bundled theme (no css file on disk)', async () => {
    const result = await themeEdit({ action: 'theme.edit', name: 'default' }, ctx)
    expect(result.success).toBe(false)
  })
})

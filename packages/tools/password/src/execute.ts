...
function buildCharset(input: PasswordInput): string {
  let chars = LOWERCASE
...
function getActiveTypeChars(charset: string, input: PasswordInput): string[] {
  const types: string[] = []
...
function applyRequireEach(chars: string[], charset: string, input: PasswordInput): string[] {
  const result = [...chars]
...
function generateNoRepeat(charset: string, input: PasswordInput): string[] {
  const { length } = input
